...
export async function listPreparedProducts({ limit, offset, sort = 'created_at', order = 'desc' }, db) {
  const result = await runDbQuery(db, {
    name: 'products.list',
    text: `SELECT * FROM prepared_products ${buildOrderBy(SORTABLE, sort, order, ORDER_BY_OPTS)} LIMIT $1 OFFSET $2`,
    values: [limit, offset],
  });
