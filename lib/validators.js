...
const CreateSaleSchema = {
  ...
  properties: {
    ...
    status: { type: 'string', enum: ['ordered', 'prepared', 'delivered', 'paid', 'cancelled'] },
...
// Define SaleCredit request schema (Phase-13): partial payments against a sale
...
const validators = {
  ...
  SaleCreditCreate: ajv.compile(CreateSaleCreditSchema),
  // Request validators
  ...
  // utility: format AJV errors into compact objects
  formatErrors: (errs) => {
    if (!errs) return [];
    return errs.map((e) => ({ instancePath: e.instancePath || '', message: e.message || '', keyword: e.keyword || '', params: e.params || {}, schemaPath: e.schemaPath || '' }));
  }
};
